...
<table border=0 width="40%" align="left">
  ...
  <tr>
	...
	echo '<select name="an">';
	echo '<option selected> - </option>';
	for ($i=1940; $i<=1980; $i++) echo '<option>'.$i.'</option>';
	echo '</select>';
	?>
	</td>
  </tr>
...
foreach ($link->query($query) as $row) {
    echo '<option value="'.$row['cod'].'">'.$row['judet'].'</option>';
}
mysqli_close($link);
?>
    ...
    <tr>
    <td>Newsletter* :</td>
    <td>Email:<INPUT TYPE="checkbox" NAME="mail" value="mail"> Posta:<INPUT TYPE="checkbox" NAME="posta" value="posta">
	Fax:<INPUT TYPE="checkbox" NAME="fax" value="fax">
	</td>
  </tr>
    </tr>
    <tr>
